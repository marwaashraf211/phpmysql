<?php
 ...
 class Connection{
    ...
    {
       $pdo= $this->pdo=new PDO('mysql:server=localhost;dbname=notes','root','');
        $pdo=$this->pdo->setAttribute(PDO::ATTR_ERRMODE,PDO::ERRMODE_EXCEPTION);
    }

    public function getNotes()
    {
        $statement=$this->pdo->prepare("SELECT * FROM notes ORDER BY create_date ");
        $statement->execute();
    ...
    public function updateNote($id,$note)
    {
        ...
        $statement->bindValue('id',$id);
        $statement->bindValue('title',$note['title']);
        $statement->bindValue('id',$note['description']);
        return $statement->execute();
